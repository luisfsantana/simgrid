  ...
  for(i = 0; i < num_arrived_tasks;i++){
    ...
    switch(policy){
      ...
      case UNICEF:
	    ...
	    break;
      case CANDIDATE1:
          h_values[i] = (0.0000147759 * sqrt(req[i])) / (56.3113236985 * (1.0 / cores[i])) + (0.0067364627 * log10(submit[i])); //256nodes
          //h_values[i] = (0.0001352 * sqrt(runtimes[i])) / (3.6161753 * (1.0 / cores[i])) + (0.0071470 * log10(submit[i])); //64nodes          
          break;
      case CANDIDATE2:
          h_values[i] = (0.0000000586 * req[i]) / (54.3772719722 * (1.0 / cores[i])) + (0.0073580360 * log10(submit[i])); //256nodes
          //h_values[i] = (0.0000253 * runtimes[i]) / (114.7085220 * (1.0 / cores[i])) + (0.0072013 * log10(submit[i])); //64nodes   
          break;
      case CANDIDATE3:           
          //h_values[i] = (0.0003358237 * runtimes[i]) * (0.0000307840 * sqrt(cores[i])) + (0.0077680112 * log10(submit[i])); //256nodes V2
          h_values[i] = (7.8011318719 * log10(req[i])) / (3647.8973260887 * sqrt(cores[i])) + (0.0066586887 * log10(submit[i]));
          //h_values[i] = (0.0016642 * log10(runtimes[i])) + (0.0005405 * log10(cores[i])) + (0.0063172 * log10(submit[i])); //64nodes         
          break;
      case CANDIDATE4:
          //h_values[i] = (0.0000132827 * sqrt(runtimes[i])) * (0.3380766088 * sqrt(cores[i])) + (0.0063685033 * log10(submit[i])); //256nodes V2         
          h_values[i] = (0.0000018357 * log10(req[i])) * (0.6789409323 * cores[i]) + (0.0071941865 * log10(submit[i])); //64nodes     
          //h_values[i] = (0.0000002 * runtimes[i]) + (0.0003207 * sqrt(cores[i])) + (0.0069554 * log10(submit[i])); //64nodes
          break;
      case CANDIDATE5:                   
          //h_values[i] = (0.000092 * log10(runtimes[i])) * (0.099621 * cores[i]) + (0.006751 * log10(submit[i])); //256nodes v2
          h_values[i] = (-0.0032553782 * req[i]) * (-0.0000001598 * cores[i]) + (0.0068844904 * log10(submit[i])); //256nodes v1
          //h_values[i] = (0.0000342717 * sqrt(runtimes[i])) * (0.0076562110 * cores[i]) + (0.0067364626 * log10(submit[i])); //256nodes v1          
          //h_values[i] = (0.0000315 * sqrt(runtimes[i])) + (0.0000040 * cores[i]) + (0.0069948 * log10(submit[i])); //64nodes
          break;
      case CANDIDATE6:
          //h_values[i] = (-0.043522 * sqrt(runtimes[i])) * (-0.000005 * cores[i]) + (0.007067 * log10(submit[i])); //256nodes v2          
          h_values[i] = (-0.0000101580 * sqrt(req[i])) * (-0.0142625801 * cores[i]) + (0.0069582192 * log10(submit[i])); //256nodes v1
          //h_values[i] = (-0.0076000 * log10(runtimes[i])) * (-0.0918610 * log10(cores[i])) + (0.0070696 * log10(submit[i])); //64nodes
          break;   
    }
  if(VERBOSE)
    XBT_INFO("Score for \"Task_%d\" [r=%.1f,c=%d,s=%d,req=%.1f]=%.7f", orig_pos[i], runtimes[i], cores[i], submit[i], req[i], h_values[i]);	
  }
...
if(VERBOSE)
XBT_INFO("Got %d nodes and %d tasks to process", number_of_nodes, number_of_tasks);
...
{                             /*  Task creation */
    ...
    for (i = 0; i < number_of_tasks; i++) {      
      ...
      do{
        ...
        sortTasksQueue(&all_runtimes[i], &all_cores[i], &all_submit[i], &all_req_runtimes[i], &orig_task_positions[i], chosen_policy, number_of_tasks - i, i);        
          }
        ...
        if(available_nodes < all_cores[i]){
          if(VERBOSE)
            XBT_INFO("Insuficient workers for \"Task_%d\" [r=%.1f,c=%d,s=%d,req=%.1f] (%d available workers. need %d). Waiting.", orig_task_positions[i], all_runtimes[i], all_cores[i], all_submit[i], all_req_runtimes[i], available_nodes, all_cores[i]);
      ...
      */
      ...
      if(VERBOSE)
      XBT_INFO("Dispatching \"%s\" [r=%.1f,c=%d,s=%d,req=%.1f]", todo[i]->name, all_runtimes[i], all_cores[i], all_submit[i], all_req_runtimes[i]);
                     ...
                     const char *application_file)
{
  ...
  double sumWaitTime = 0.0f;
  double sumTurnaroundTime = 0.0f;
  ...
  for (i = NUM_TASKS_STATE; i < number_of_tasks; i++) {
    ...
    double turnaroundTime = waitTime + runTime;
  ...
  double AVGWaitTime = sumWaitTime / (number_of_tasks - NUM_TASKS_STATE);
  double AVGTurnaround = sumTurnaroundTime / (number_of_tasks - NUM_TASKS_STATE);
...
int main(int argc, char *argv[])
{
  ...
  if(argc >= 4){
    for(i = 3;i < argc; i++){
      ...
      if (strcmp(argv[i], "-c5") == 0){
        chosen_policy = CANDIDATE5;
      }
      if (strcmp(argv[i], "-c6") == 0){
        chosen_policy = CANDIDATE6;
      }
  ...
  if(number_of_tasks == 0){
    printf("Invalid number_of_tasks parameter. Please set -nt value in runtime.\n");
    exit(1);
  }
