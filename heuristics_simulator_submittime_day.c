...
#define NUM_TASKS_STATE 16 /*quantidade de tarefas de warmaup */
...
int num_cores_busy=0;
int num_sched_events=0;
...
void sortTasksQueue(double* runtimes, int* cores, int* submit, int* orig_pos, int policy, int queue_num_tasks, int num_tasks_disp){
	...
	for(i = 0; i < num_arrived_tasks;i++){
		...
		case UNICEF:
			...
			break;
		case CANDIDATE1:
			h_values[i] = (0.0000147759 * sqrt(runtimes[i])) / (56.3113236985 * (1.0 / cores[i])) + (0.0067364627 * log10(submit[i])); //256nodes
			//h_values[i] = (0.0001352 * sqrt(runtimes[i])) / (3.6161753 * (1.0 / cores[i])) + (0.0071470 * log10(submit[i])); //64nodes
			break;
		case CANDIDATE2:
			h_values[i] = (0.0000000586 * runtimes[i]) / (54.3772719722 * (1.0 / cores[i])) + (0.0073580360 * log10(submit[i])); //256nodes
		...
		case CANDIDATE3:
			...
			h_values[i] = (0.0058349125 * log10(runtimes[i])) +
			(0.0014261611 * log10(cores[i])) +
			(0.0064584213 * log10(submit[i])) +
			(0.0011193405 * (log10(runtimes[i]) * log10(cores[i]))) +
			(-0.0012252658 * (log10(runtimes[i]) * log10(submit[i]))) +
			(-0.0013848787 * (log10(cores[i]) * log10(submit[i]))); //256nodes v1
		...
		case CANDIDATE4:
			...
			h_values[i] = (0.0000018357 * log10(runtimes[i])) * (0.6789409323 * cores[i]) + (0.0071941865 * log10(submit[i])); //64nodes
		...
		case CANDIDATE5:
			...
			h_values[i] = (-0.0032553782 * runtimes[i]) * (-0.0000001598 * cores[i]) + (0.0068844904 * log10(submit[i])); //256nodes v1
		...
		case CANDIDATE6:
			...
			h_values[i] = (-0.0000101580 * sqrt(runtimes[i])) * (-0.0142625801 * cores[i]) + (0.0069582192 * log10(submit[i])); //256nodes v1
		...
		for (i = 0; i < number_of_tasks; i++) {
			...
			do{
				if(i >= NUM_TASKS_STATE){
					sortTasksQueue(&all_runtimes[i], &all_cores[i], &all_submit[i], &orig_task_positions[i], chosen_policy, number_of_tasks - i, i);
				}
                     ...
                     const char *application_file)
{
	...
	double AVGUtilization = num_cores_busy / num_sched_events;
	...
	if(VERBOSE){
		...
		printf("%f,%f,%f,%f\n", AVGSlowdown, AVGWaitTime, AVGTurnaround, AVGUtilization);
	}
	//int i,j;
